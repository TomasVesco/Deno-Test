...
const DB_USERS: User[] = [];
DB_USERS.push({ uuid: "1", name: "Alan 'the beast' Sinicco", birthDate: new Date() });
DB_USERS.push({ uuid: "2", name: "Tomás 'Pixel' Vesco", birthDate: new Date() });
...
export const updateUser = async (ctx: Context) => {
  try {
    ...
    if (userIndex) {
      ...
      ctx.response.body = {
        code: "00",
        data: { uuid: userId, name, birthDate },
      };
    } else {
      ctx.response.body = {
        code: "01",
        msg: `Usuario with ID: ${userId} do not exist.`,
      };
    }
  } catch (error) {
    ctx.response.status = 500;
...
export const deleteUser = async (ctx: Context) => {
  try {
    ...
    if (userIndex !== -1) {
      ...
      ctx.response.body = {
        code: "00",
        msg: `User with ID: ${userId} eliminated.`,
      };
    } else {
      ctx.response.body = {
        code: "01",
